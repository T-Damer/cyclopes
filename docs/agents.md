...
Load `dist/` at `chrome://extensions` → Developer mode → Load unpacked. Reload a local fixture page once, click the Cyclopes icon, and verify anchored ASCII loading badges follow scrolling/transforms, confidence badges share image stacking, no content blur, and 32×32 icons, video posters, and images over 85% occluded are ignored. Field cases are frozen evaluation-only data and must never influence training, calibration, threshold, or checkpoint selection. No live Rule34 check is part of the vNext training gate.
